    ...
    <div className="my-8 max-w-prose text-xl mx-auto">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        {posts.slice(0, 3).map((post) => (
          <div className="group bg-muted overflow-hidden rounded-lg" key={post.id}>
            <Link href={`/blog/${post.slug}`}>
              <AspectRatio ratio={16 / 9} className="w-full">
                <Image
                  ...
                  alt={post.title}
                  fill
                  className="group-hover:brightness-75 h-full min-h-full min-w-full object-cover object-center"
                />
              </AspectRatio>
            </Link>
            <div className="prose prose-sm dark:prose-invert p-4">
              <h3 className="line-clamp-2 hover:underline cursor-pointer">{post.title}</h3>
